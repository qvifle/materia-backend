...
router.post("/:projectId", async (req, res) => {
    ...
    const isAlreadyMember = !!recipient.memberProjects.find((project) => project.id === projectId);
    if (isAlreadyMember) {
        res.status(409).send("User already member");
        return;
    }
    const isAlreadyInvited = !!recipient.receivedInvites.find((invite) => invite.projectId === projectId);
    if (isAlreadyInvited) {
...
router.patch("/:inviteId/accept", async (req, res) => {
    ...
    const invite = user.receivedInvites.find((invite) => invite.id === inviteId);
    if (!invite) {
...
router.delete("/:inviteId/reject", async (req, res) => {
    ...
    const invite = user.receivedInvites.find(({ id }) => id === inviteId);
    if (!invite) {
        res.status(404).send("Invite not found");
        return;
    }
...
router.delete("/:inviteId", async (req, res) => {
    ...
    const invite = sender.sentInvites.find(({ id }) => id === inviteId);
    if (!invite) {
